...
% --- Tytuł ---
{\color{nustblue}\bf
\Huge DINOmite: Adversarial Robustness \\[0.3em] of DINOv3 Vision Transformers
}
...
\begin{itemize}
    ...
    \item \textbf{Goal:} Misclassification with minimal perturbation.
\end{itemize}
\end{posterbox}
...
\begin{itemize}
    \item \textbf{Backbone:} DINOv3 ViT-S/16 (Frozen for Classification).
...
\begin{itemize}
    \item \textbf{Metric:} Clean vs. Robust Accuracy (PGD-40).
    \item \textbf{Delta:} $\delta \in \{0, \dots, 8/255\}$.
    \item \textbf{Attacks:} FGSM, PGD, C\&W.
    \item \textbf{Defenses:} PGD-AT, TRADES.
\end{itemize}
\end{posterbox}
...
\begin{itemize}
    \item \textbf{Adversarial Training Effectiveness:} DINO-pretrained ViTs are vulnerable to attacks like PGD. However, defenses like PGD-AT or TRADES significantly enhance robustness, mirroring Madry et al.'s findings for CNNs.
    \item \textbf{Trade-offs:} Increased robustness introduces a performance penalty on clean data, causing a noticeable drop in standard accuracy.
    \item \textbf{Visual Differences:} C\&W attacks generate perturbations that are less perceptible to the human eye than those produced by PGD and FGSM.
\end{itemize}

\subsection*{Future Work}
\begin{itemize}
    \item Evaluating Certified Robustness.
    \item Scaling to ImageNet-1k and GTSRB.
...
\textit{Note: The strongest defense benchmark. We use 40 steps.}
...
$$ \min_\theta \mathbb{E}_{(x,y)\sim D} [\max_{\|\delta\|_\infty \le \epsilon} L(f_\theta(x+\delta), y)] $$
\textit{Key Idea: Train directly on the worst-case examples.}
...
$$ \min_\theta \mathbb{E} [\underbrace{L(f(x),y)}_{\text{Natural}} + \beta \cdot \underbrace{\text{KL}(f(x) \| f(x+\delta))}_{\text{Robustness}}] $$
\textit{Key Idea: Enforce smoothness of the decision boundary.}
\end{posterbox}
...
\begin{posterbox}[name=results, column=2, below=comparison, above=references]{Quantitative Results}
Comparison on CIFAR-10 ($\epsilon=8/255$).

\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Model} & \textbf{Clean} & \textbf{PGD} \\
\midrule
Standard & \textbf{96.56\%} & 0.00\% \\
PGD-AT & 77.19\% & \textbf{40.62\%} \\
TRADES & 81.25\% & 35.94\% \\
\bottomrule
\end{tabular}
\end{center}
